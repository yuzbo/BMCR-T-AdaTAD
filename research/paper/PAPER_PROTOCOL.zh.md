...
|干预监督|真实 frame/T/D/S 重执行整个 student、全局 TIA、decoder、检测头|feature repair 只作代理；不把 repair 收益直接当真实执行收益|
...
六项远端 CPU 验证已通过，包括真实 24 层执行/算子计数、混合预算反向传播的 TIA 时间轴、可训练 head 与冻结参照、ANet 坐标与重复物理帧、预算约束。S/B point、S/B TadTR、S 全骨干微调、S 官方 MAE decoder、ANet-S 这七个可用组合通过实际权重构建。GPU 上的真实长课程与完整结果仍须看权威 deployment.json，登记配置与进入 Slurm 队列分别报告。
