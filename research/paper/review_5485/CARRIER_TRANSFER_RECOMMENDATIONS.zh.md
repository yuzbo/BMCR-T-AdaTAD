...
还需避免一个不同形式的双零问题：若Cross读取完整的非零 `C_L`，新增零输出投影可先获得梯度，随后带动上游Q；但若读取 `C_L-C0`，且Q末层与Cross新增输出投影同时为零，则差分输入和读出权重都为零，在没有其他非零监督路径时，该支路两端都无法获得启动梯度。因此不能笼统地将所有新增映射都置零。应明确选择完整状态输入，或只在一侧设置零初始化，并在相同RGB/support下检查旧输出是否保持、新增参数是否能启动学习。
...
- 方法线程的完整短答已保存为 `CARRIER_PEER_REPLY.zh.md`，已完整阅读并吸收其关于差分支路双零初始化的补充。
- 对方报告的完整THUMOS、seed42、60轮终点内部消融为：U-static→U-prog，B为56.1546→59.4518%，S为53.1331→55.6313%。这属于其原K16 clip支持协议，仅支持该协议内progressive更新的单seed正向线索，不是超过当前Cross或迁移成功的证据。
- 对方提供了三个coupling点下P/Q与全轴TIA的有条件算式小计，并明确额外总FLOPs未实现/实测。该小计没有覆盖新增cheap特征来源、反馈Up/Down、坐标映射与其他操作，本报告不将其用作新模型总成本或Pareto点。
