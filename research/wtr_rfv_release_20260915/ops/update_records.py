"""Record the completed RFV mini experiments without changing their raw receipts."""
import json
...
source=workspace/'h65_clean_adatad/rfv_sprint_20260915/research/rfv_sprint_20260915'
queue=json.loads((out/'EXPERIMENT_QUEUE.json').read_text(encoding='utf8'))
queue['plan']='RFV Fast Sprint v4: measured-evidence revision'
queue['scientific_sources'].update(
    rfv_capture20_60='3a7e93f3f77350e9598011db1dd821cedaee8f0b',
    g0b_postprocess='dca6564e77e13717adf461dcf1feabf8bc18bbda',
    action_holdout_and_B0='d62ea556c56ca6157289a866aaafc79c4fa427c6')
queue['resources']={
    'snapshot':'cluster_queue_latest.json; value_diagnostics_status.json',
    'AutoDL_GPU0':'Atlas-S released; RFV B0 PID280193 completed; no current RFV GPU work',
    'AutoDL_GPU1':'RFV replay PID272682 completed; B preserved459/792 and HELD',
    'AutoDL_CPU':'Action holdout PID280190 completed; next existing-bank descriptor diagnosis only',
    '4090':'D-V1290651/D-U1290652/S-V1290654/S-U1290655 RUNNING at16:04; keep405 courses',
    'A100':'SSH connected; own Slurm queue empty at16:04; distinct work after scientific admission'}
queue['delivered'][1]['scope']='one fit video/all3 windows, actual evaluator/JSON/metrics/no-GT; formal recipe2-update/reload still required'
queue['delivered'][-1].update(actionable_states={'fit':25,'calibration':8,'inner_holdout':10},actions=688)
queue['decisive_outputs']=[
    dict(file='T_LOCAL_CF.json',status='PASS',scope='headroom only; training-sidecal20/all46 windows',
         ap_delta_pp=.52841322355,ap_delta_ci95_pp=[.08244042796,.71468687374],ap07_delta_pp=.82774438028,
         next='No repeat; prerequisite, not course authorization'),
    dict(file='GRAPH_G1_T_MINI.json',status='FAIL_MINI',scope='Plain learnability and Graph value gate; not universal failure',
         plain_m_regret=.00368165746,plain_l_regret=.00395215452,static_regret=.00338686531,
         static_minus_plain_l_ci95=[-.0012700129,.0000951362],next='Resolve learnability before task-level Graph'),
    dict(file='ACTION_HOLDOUT_DIAGNOSTIC.json',status='COMPLETE_NO_COVERAGE_UNLOCK',pid=280190,
         scope='existingfit25, fixed8/8 actions, CPU2000steps x3seeds',held_spearman=.04444444444,
         held_regret=.00149207135,stop_regret=.00141353726,new_cf_queries=0,
         next='One descriptor identifiability analysis; no32-to64 increment yet'),
    dict(file='RISE_A0_ALL.json',status='MEASURED',scope='historical20/40/60; no newT orFVD claim',
         rho20_60=.776412,topk20_60=.726744,sign_flip20_60=.143895,
         next='Reuse runner on newT trajectory only afterT course admission'),
    dict(file='RISE_B_T.json',status='FAIL_HISTORICAL_B0',pid=280193,beta=1.1,
         current_post_regret=.003277700146,true_ema_regret=.003263545533,future_regret=.003277700146,
         forecast_value_gate=False,fvd_unlocked=False,next='NoFVD training; currentValue learnability first'),
    dict(name='Descriptor identifiability diagnostic',status='NEXT_CPU_ONLY',
         input='existingmini fit bank and locked8/8 split',
         scope='row-action mapping, exact/near descriptor versus actualgain; no new router, fitting sweep orGPU labels'),
    dict(name='MatchedT-U/T-V80',status='WAITING_LEARNABILITY',
         next='Headroom AND Plain learnability AND complete recipe technical admission; theninline milestones'),
    dict(name='T-V-GCTX / T-V-FVD / T-V-GCTX-FVD',status='WAITING_SEPARATE_GATES',
         next='Respect mechanism andtask evidence; no launch fromconfig existence')]
queue['course_unlocks']['T-V-FVD']='NewT actualdrift AND same-state forecast beatCurrent/Post/TrueEMA; beta1 control; historicalB0 never directly unlocks'
queue['stop_rules'].insert(2,'Within-video held actions fail: no automatic32-to64 expansion')
queue['atlas_freeze']='Atlas-S GPUqueries frozen; CPUstats/plots/export only. BHELD. Cross-axis interactions remainexplicit gap.'
queue['final_model']='NOT_PROVEN; no mechanism has task-levelPASS orFinal=yes'
(out/'EXPERIMENT_QUEUE.json').write_text(json.dumps(queue,ensure_ascii=False,indent=2)+'\n',encoding='utf8')

state=out/'EXECUTION_STATE.zh.md'
archive=out/'EXECUTION_STATE.history_before_completed_B0.zh.md'
if not archive.exists():shutil.copy2(state,archive)
state.write_text('''# RFV Fast Sprint 当前执行状态
...
for target in [source/'PROTOCOL.zh.md',workspace/'reports/wtr_fasttrack_20260915/EXPERIMENT_PLAN.zh.md']:
    shutil.copy2(out/'EXPERIMENT_PLAN.zh.md',target)
for name in ['EXPERIMENT_QUEUE.json','EXECUTION_STATE.zh.md']:
    shutil.copy2(out/name,source/name)
ledger=workspace/'reports/wtr_fasttrack_20260915/FINAL_MODEL_LEDGER.csv'
for target in [out/'FINAL_MODEL_LEDGER.csv',source/'FINAL_MODEL_LEDGER.csv']:
    shutil.copy2(ledger,target)
print('Updated plan, queue, execution state and ledger copies')
