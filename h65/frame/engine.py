...
class PackedStateEngine(nn.Module):
    ...
    def forward(self,vit,clips,policy,native_valid=None):
        ...
        if depth!=12 or 0 in policy.mod_layers or depth-1 in policy.mod_layers:raise ValueError('A-MoD contract:12 layers with dense first/last')
        ...
        tr=empty_trace(depth);tr['qk_av_macs']=[0]*depth;previous_scores=None
        tr['query_masks']=[]
        ...
        for i,block in enumerate(vit.blocks):
            if i not in keep_layers:
                tr['depth_masks'].append(torch.zeros_like(valid));tr['spatial_masks'].append(torch.zeros_like(valid));tr['query_masks'].append(torch.zeros_like(valid));continue
        ...
        tr.update(patch_input_shape=list(clips.shape),physical_candidates=a*2,
                  ...
                  amod_full_kv=policy.amod_full_kv)
        return x,h,w,tr
