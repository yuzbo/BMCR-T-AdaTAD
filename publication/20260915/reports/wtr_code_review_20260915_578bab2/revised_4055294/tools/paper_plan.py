...
def configurations():
    base=dict(recipe=RECIPE,dataset='thumos',backbone='s',head='point',seed=42,epochs=80,eval_epochs=[10,20,40,60,80],
    ...
    rows=[]
    def add(name,backbones=('s',),seeds=(42,),dataset='thumos',epochs=40,head='point',**changes):
        for backbone,seed in itertools.product(backbones,seeds):
            ...
            rows.append(cfg)
    add('full',('s','b'),(42,),epochs=80)
    add('uniform',('s','b'),(42,),epochs=80,selector='uniform',frame_utility=False)
...
def build_plan():
    ...
    for cfg in configs:
        ...
        priority=10 if cfg['comparison']=='full' and cfg['seed']==42 else 20 if cfg['comparison'] in ('uniform','dense') and cfg['seed']==42 else 30 if cfg['seed']!=42 else 40
        ...
        if cfg['comparison']=='full' and cfg['seed']==42:
    ...
    for cfg in configs:
        if cfg['dataset']!='thumos' or cfg['comparison']!='full' or cfg['seed']!=42:continue
        ...
        for epoch in ([40,80] if cfg['epochs']==80 else [40]):
            ...
            stages[name]=dict(kind='analysis',priority=5,dependencies=[],assets=[],
                ...
                args=['tools/frame_errors.py','--predictions',f'{current}/result_detection.json','--reference-predictions',f'{other}/result_detection.json',
                      '--output',f'research/paper/runs/{name}','--replicates','1000','--seed','42'])
    return dict(recipe=RECIPE,configs=configs,stages=stages,independent_launch=True,performance_gates=False,
        data_and_technical_dependencies_only=True,legacy_preserved=True,required_seed=42,runs_per_configuration=1,max_live_jobs=10,max_live_train=8,
